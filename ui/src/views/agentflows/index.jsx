...
import CreateChatflowCard from '../chatflows/CreateChatflowCard'
...
const Agentflows = () => {
    ...
    const addNew = () => {
        navigate(navigationPaths.workspace.agentflows.new())
    }

    const goToCanvas = (selectedAgentflow) => {
        navigate(navigationPaths.workspace.agentflows.detail(selectedAgentflow.id))
    }

    const handleImportAgentflow = (fileData) => {
        try {
            const flowData = getWorkflowDataFromJSON(fileData, 'MULTIAGENT')
            ...
            dispatch(
                enqueueSnackbarAction({
                    message: `Failed to import agentflow: ${error.message}`,
                    options: {
                    ...
                    {!view || view === 'card' ? (
                        <>
                            {isLoading && !getAllAgentflows.data ? (
                                ...
                                <Box display='grid' gridTemplateColumns='repeat(3, 1fr)' gap={gridSpacing}>
                                    <CreateChatflowCard onFileUpload={handleImportAgentflow} isAgentCanvas={true} />
                                    {getAllAgentflows.data?.filter(filterFlows).map((data, index) => (
                                        <ItemCard key={index} onClick={() => goToCanvas(data)} data={data} images={images[data.id]} isShowCardOptions={true} />
                                    ))}
                                </Box>
                            )}
                        </>
