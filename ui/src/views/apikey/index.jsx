    ...
    return (
        <>
            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <StyledTableCell>
                    {props.apiKey.linkedChatFlows.length}{' '}
                    {props.apiKey.linkedChatFlows.length > 0 && (
                        <IconButton aria-label='expand row' size='small' color='inherit' onClick={() => setOpen(!open)}>
                            {props.apiKey.linkedChatFlows.length > 0 && open ? <IconChevronsUp /> : <IconChevronsDown />}
                        </IconButton>
                    )}
                </StyledTableCell>
                            ...
                            <Box sx={{ borderRadius: 2, border: 1, borderColor: theme.palette.grey[900] + 25, overflow: 'hidden' }}>
                                <Table aria-label='chatflow table'>
                                    <TableHead sx={{ height: 48 }}>
                                        <TableRow>
                                            <StyledTableCell sx={{ width: '25%' }}>Chatflow Name</StyledTableCell>                                            
                                            <StyledTableCell sx={{ width: '25%' }}>Category</StyledTableCell>
                                            <StyledTableCell sx={{ width: '25%' }}>Type</StyledTableCell>
                                            <StyledTableCell sx={{ width: '25%' }}>Modified On</StyledTableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {props.apiKey.linkedChatFlows.map((flow, index) => (
                                            <TableRow key={index}>
                                                <StyledTableCell>
                                                    <Link
                                                        href={flow.type === 'CHATFLOW' 
                                                            ? navigationPaths.workspace.chatflows.detail(flow.id)
                                                            : navigationPaths.workspace.agentflows.detail(flow.id)}
                                                        target='_blank'
                                                        underline='always'
                                                ...
                                                <StyledTableCell>{flow.type}</StyledTableCell>
...
const APIKey = () => {
    ...
    const deleteKey = async (key) => {
        const confirmPayload = {
            title: `Delete`,
            description:
                key.linkedChatFlows.length === 0
                    ? `Delete key [${key.keyName}] ? `
                    : `Delete key [${key.keyName}] ?\n There are ${key.linkedChatFlows.length} chatflows using this key.`,
