...
const AssistantDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    const [disableDeleteBoth, setDisableDeleteBoth] = useState(true)

    useEffect(() => {
        setDisableDeleteBoth(!assistantCredential)
    }, [assistantCredential])
    ...
    useEffect(() => {
        if (getSpecificAssistantApi.data) {
            ...
            setAssistantCredential(getSpecificAssistantApi.data.credentialId)
    ...
    useEffect(() => {
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            ...
            setAssistantCredential(dialogProps.data.credentialId)
    ...
    const addNewAssistant = async () => {
        ...
        try {
            ...
            const obj = {
                ...
                credentialId: assistantCredential
            }
    ...
    const deleteAssistant = async (isDeleteRemote) => {
        setDeleteDialogOpen(false)
        try {
            const delResp = await assistantsApi.deleteAssistant(assistantId, isDeleteRemote)
            if (delResp.data) {
                enqueueSnackbar({
                    message: delResp.data.message,
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
    ...
    const component = show ? (
        ...
        >
            ...
            <DeleteConfirmDialog
                ...
                disableDeleteBoth={disableDeleteBoth}
            />
