...
import { FileConvertToBase64Component } from '@/ui-component/file/FileUploadBase64'
...
import { InputTooltip } from './CustomTooltip'
...
const NodeInputHandler = ({
    ...
    return (
        <div ref={ref}>
            {inputAnchor && (
                <>
                    <InputTooltip title={inputAnchor.type}>
                        <EnhancedHandle
                            nodeData={data}
                            type='target'
                            ...
                            id={inputAnchor.id}
                            nodeId={data.id}
                            handleId={inputAnchor.id}
                            isSelected={selectedHandle?.handleId === inputAnchor.id && isNodeSelected}
                            onClick={handleClick}
            ...
            {((inputParam && !inputParam.additionalParams) || isAdditionalParams) && (
                <>
                    {inputParam.acceptVariable && !isAdditionalParams && (
                        <InputTooltip title={inputParam.type}>
                            <EnhancedHandle
                                nodeData={data}
                                type='target'
                                position={Position.Left}
                                ...
                                id={inputParam.id}
                                nodeId={data.id}
                                handleId={inputParam.id}
                                isSelected={selectedHandle?.handleId === inputParam.id && isNodeSelected}
                                onClick={handleClick}
                                isValidConnection={(connection) => isValidConnection(connection, reactFlowInstance)}
                                style={{
                                    height: 10,
                    ...
                    <Box sx={{ p: disablePadding ? 0 : 2 }}>
                            ...
                            (inputParam.name === 'template' || inputParam.name === 'systemMessagePrompt') && (
                                <>
                                    <Button
                                        ...
                                        disabled={disabled}
                                        sx={{ borderRadius: '4px', width: '100%', mb: 2, mt: 0 }}
                                        variant='outlined'
                        ...
                        {inputParam.type === 'credential' && (
                            <CredentialInputHandler
                                ...
                                onSelect={(newValue) => {
                                    data.credentialId = newValue
                                    data.inputs[BITFLOW_CREDENTIAL_ID] = newValue // in case data.credentialId is not updated
                                }}
                            />
                        )}
                        ...
                        {inputParam.type.includes('conditionFunction') && (
                            <>
                                <Button
                                    ...
                                    sx={{ borderRadius: '4px', width: '100%', mt: 1 }}
                                    variant='outlined'
